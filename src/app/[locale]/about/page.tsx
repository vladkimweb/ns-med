...
export default function About() {
  return (
    <>
      <PageHero title="О НАС" subtitle="Профессиональная медицинская координация и поддержка" />
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {teamMembers.map((member) => (
              <div
                key={member.id}
                className="bg-white p-6 rounded-xl shadow-md border border-gray-200 text-center hover:shadow-lg transition-shadow duration-300"
              >
...
function InfoCard({ title, text }: { title: string; text: string[] }) {
  return (
    <div className="bg-white p-6 rounded-xl shadow-md border border-gray-50 space-y-4">
...
function ServiceCard({ title, description }: { title: string; description: string }) {
  return (
    <div className="bg-white p-6 rounded-xl shadow-md border border-gray-50 hover:shadow-lg transition duration-300">
